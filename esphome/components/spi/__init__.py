...
SPI_DATA_RATE_OPTIONS = {
    ...
    32e3: SPIDataRate.DATA_RATE_32KHZ,
...
def spi_device_schema(
    ...
    schema = {
        ...
        cv.Optional(CONF_SPI_ROLE, default=default_role): cv.enum(
            SPI_ROLE_OPTIONS, lower=True
        ),
...
async def register_spi_device(var, config):
    ...
    if CONF_SPI_ROLE in config:
        cg.add(var.set_role(config[CONF_SPI_ROLE]))
